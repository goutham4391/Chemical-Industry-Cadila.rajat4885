var slidebars=function(){var canvas=$('[canvas]'),offCanvas={},initialized=!1,registered=!1,sides=['top','right','bottom','left'],styles=['reveal','push','overlay'],registerSlidebar=function(id,side,style,element){if(offCanvas.hasOwnProperty(id)){throw "Error attempting to register Slidebar, a Slidebar with ID '"+id+"' already exists."}
offCanvas[id]={'id':id,'side':side,'style':style,'element':element,'active':!1}},getAnimationProperties=function(id){var elements=$(),amount='0px, 0px',duration=parseFloat(offCanvas[id].element.css('transitionDuration'),10)*1000;if(offCanvas[id].style==='reveal'||offCanvas[id].style==='push'){elements=elements.add(canvas)}
...
if(!registered){$('[off-canvas]').each(function(){var parameters=$(this).attr('off-canvas').split(' ',3);if(parameters[0]&&sides.indexOf(parameters[1])!==-1&&styles.indexOf(parameters[2])!==-1){registerSlidebar(parameters[0],parameters[1],parameters[2],$(this))}else{throw "Error attempting to register Slidebar, please specifiy a valid space separated 'id side style'."}});registered=!0}
initialized=!0;this.css();$(events).trigger('init');if(typeof callback==='function'){callback()}};this.exit=function(callback){if(!initialized){throw 'You need to initialize Slidebars first.'}
var exit=function(){initialized=!1;$(events).trigger('exit');if(typeof callback==='function'){callback()}};if(this.active('slidebar')){this.close(exit)}else{exit()}};this.css=function(callback){if(!initialized){throw 'You need to initialize Slidebars first.'}
...
var open=function(){offCanvas[id].active=!0;offCanvas[id].element.css('display','block');$(events).trigger('opening',[offCanvas[id].id]);var animationProperties=getAnimationProperties(id);animationProperties.elements.css({'transition-duration':animationProperties.duration+'ms','transform':'translate('+animationProperties.amount+')'});setTimeout(function(){$(events).trigger('opened',[offCanvas[id].id]);if(typeof callback==='function'){callback()}},animationProperties.duration)};if(this.active('slidebar')&&this.active('slidebar')!==id){this.close(open)}else{open()}};this.close=function(id,callback){if(typeof id==='function'){callback=id;id=null}
...
if(id&&offCanvas[id].active){offCanvas[id].active=!1;$(events).trigger('closing',[offCanvas[id].id]);var animationProperties=getAnimationProperties(id);animationProperties.elements.css('transform','');setTimeout(function(){animationProperties.elements.css('transition-duration','');offCanvas[id].element.css('display','none');$(events).trigger('closed',[offCanvas[id].id]);if(typeof callback==='function'){callback()}},animationProperties.duration)}};this.toggle=function(id,callback){if(!initialized){throw 'You need to initialize Slidebars first.'}
...
var active=!1;if(!query){active=initialized}else if(query==='slidebar'){if(!initialized){throw 'You need to initialize Slidebars first.'}
for(var id in offCanvas){if(offCanvas.hasOwnProperty(id)){if(offCanvas[id].active){active=offCanvas[id].id;break}}}}else{if(!initialized){throw 'You need to initialize Slidebars first.'}
